...
const ArticleDetail = () => {
    ...
    const fetchNotes = async () => {
        try {
            const response = await GetNoteRequest(url);
            const formattedNotes = response.data.data.map(note => ({
                start: note.insertStart,
                end: note.insertEnd,
                text: note.note,
                color: note.color
            }));
    ...
    const handleNewNote = async ({start, end, text, color, folderId}) => {
    ...
    useEffect(() => {
        ...
        const viewCountTimer = setTimeout(() => {
            IncreaseArticleViewCountRequest(url);
        }, 5000);

        return () => clearTimeout(viewCountTimer);
    }, []);
    ...
    return (
        <div className="page-container container-fluid">
            <button
                className="comment-canvas-btn"
                onClick={() => setIsPopupOpenComment(!isPopupOpenComment)}
            >
                ...
                <div className="article-detail-info-flex">
                    <div className="my-notes-process-flex align-items-center" style={{columnGap: '5px'}}>
                        <svg clipRule="evenodd" fillRule="evenodd" strokeLinejoin="round" strokeMiterlimit="2"
                             width="30" fill="white" height="30" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path
